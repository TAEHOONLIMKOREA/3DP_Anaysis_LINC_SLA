...
## 원본 데이터를 Pandas를 이용하여 불필요 데이터는 지우고 row값을 column의 인덱스로 변경하는 전처리작업 진행
### 원본 데이터 :
![image](https://user-images.githubusercontent.com/87262811/214532433-106480a1-bb87-43b2-bfa9-78f08b46cf20.png)
##


### 1. 모든 row에 같은 값이 들어가 있는 Nickname을 제거 후 DB명으로 변경 (DB명:LINCSOLUTION_CMET_ATOMm-4000)
#### - Measurement 이름은 모델 출력 시작 날짜와 시간으로 지정 <br/>
### 2. 불필요 데이터 삭제 ( MIN_VALUE, MAX_VALUE, SENSOR_TYPE )
#### - Min,Max 값도 이미 정해져 있는 값이라 메타데이터로 정의해두면 데이터 크기를 감소시킬 수 있음
#### - SENSOR_TYPE도 이미 이름에 나와 있고 TAG keys로 정의해 두고 사용하기에 크게 도움이 안되기에 삭제 후 데이터 크기를 감소하는 방향으로 진행<br/>

### 3. pandas의 pivot_table()함수를 이용하여 반복되는 11개의 Parameter들을 Column으로 변경 하였음.
